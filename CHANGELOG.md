...
- 用户中心“我的邮箱”新增批量创建：单次 1 至 50 个、最多 5 路并发，复用现有用户归属、积分、失败退款和幂等逻辑，创建结果自动进入当前用户邮箱列表。
- 解耦 POP3 与 HTTP 邮箱会话有效期：普通用户可通过长期 `userAuthCode` 读取自己名下已过期但未释放且启用 POP 的邮箱；短期 `mailboxToken` 和验证码 HTTP API 仍按 `MAILBOX_SESSION_TTL_SECONDS` 过期。
- 文档补充已有线上部署升级流程，明确数据库自动升级到版本 5、SMTP 配置、禁止重复清理数据、管理员 POP 授权码重新保存、注册邮件验收和回滚边界。
