...
用户级 POP 授权码由用户主动设置，最小长度和复杂度按密码规则校验；完整值只在设置成功时展示一次，后续忘记时只能重新设置，不能找回旧值。
...
用户中心不显示 CloudMail 实例、域名、其他用户、请求日志和系统配置。用户邮箱列表只展示地址、用途、状态、创建时间、过期时间、验证码状态和脱敏错误，不展示内部实例凭据、邮箱内部密码、完整授权码或完整邮件正文。

### 12.2.1 用户中心操作

- 调用密钥：创建、命名、查看前缀和掩码、撤销；完整密钥只返回一次；
- POP 授权码：设置、重置、查看是否已配置；不提供旧授权码找回；管理员强制清除后，用户必须重新设置；
- 积分：查看当前余额和最近积分变更摘要，不允许用户自行增加积分；
- 邮箱：查看自己的邮箱记录、状态、过期时间和验证码状态；释放邮箱仍需经过邮箱 Token 或用户归属校验；
...
2. 在用户中心设置并安全保存用户级 `userAuthCode`；
