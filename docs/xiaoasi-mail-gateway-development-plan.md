...
10. 用户可以自行创建、查看掩码、撤销自己的调用密钥；调用密钥通过 `X-API-Key` 使用，密钥归属用户而不是管理员或系统全局。
11. 每个用户拥有一个用户级 POP 授权码；该授权码只保存哈希，用户可以在用户中心设置或重置，POP 登录时用于访问该用户自己的邮箱。
...
23. 管理员 POP 授权码与管理员登录密码、普通用户 `userAuthCode` 完全分离；管理员 POP 授权码按明文保存并允许管理员随时查看，普通用户 `userAuthCode` 仍只保存哈希。管理员强制重置普通用户授权码时只使旧值失效，不返回新授权码明文，由普通用户在用户中心点击按钮重新生成。
...
POP 授权码不再按邮箱保存，而是校验邮箱所属用户的 `user_auth_code_hash`。同一用户的所有有效邮箱共用该用户级授权码；用户重置授权码后，旧授权码立即对该用户的全部邮箱失效。CloudMail 内部邮箱密码仍由网关生成，不返回调用方，也不写入普通日志。
...
| `key_hash` | TEXT | 调用密钥哈希，不保存明文 |
| `enabled` | BOOLEAN | 是否可用 |
| `last_used_at` | DATETIME | 最近使用时间 |
| `created_at` | DATETIME | 创建时间 |
| `revoked_at` | DATETIME | 撤销时间 |

完整调用密钥只在创建成功响应中返回一次。用户可以创建多个密钥、查看名称和掩码、撤销自己的密钥，但不能查看其他用户密钥；首版不设计密钥级复杂 scopes，所有用户密钥都只能访问该用户自己的业务资源。
...
用户级 POP 授权码由用户中心使用浏览器安全随机数自动生成，用户不手动输入或重复确认；完整值只在生成成功时展示一次，后续忘记时只能重新生成，不能找回旧值。
...
| `X-API-Key` | 用户外部调用网关业务 API | 用户主动撤销前有效 | 只保存调用密钥哈希 |
| `mailboxToken` | 单个邮箱状态、验证码、释放接口 | 短期 | 网关签名，无状态校验 |
| `userAuthCode` | 该用户全部邮箱的 POP3 登录密码 | 用户重置前有效 | 只保存用户级哈希 |
| `adminPopAuthCode` | 唯一管理员读取全部邮箱的 POP3 登录密码 | 管理员重置前有效 | 只保存管理员用户记录中的哈希 |
...
用户中心不显示 CloudMail 实例配置、其他用户、请求日志和系统配置。用户邮箱列表展示地址、用途/来源、域名、状态、创建时间、过期时间和已识别验证码，支持按邮箱/域名/来源搜索，并按用途、邮箱状态、验证码状态筛选；不展示内部实例凭据、邮箱内部密码、完整授权码或完整邮件正文。

### 12.2.1 用户中心操作

- 调用密钥：创建、命名、查看前缀和掩码、撤销；完整密钥只返回一次；
- POP 授权码：自动生成、重置、查看是否已配置；不提供旧授权码找回；管理员强制清除后，用户必须重新生成；
