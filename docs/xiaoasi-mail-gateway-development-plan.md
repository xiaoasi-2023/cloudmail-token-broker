...
- 状态：核心网关、用户中心、邮箱验证码注册、用户级调用密钥、用户级 POP 授权码、管理员全局 POP 授权码、积分扣费、CDK 套餐/生成/禁用/兑换、管理端权限入口和最小只读 POP3 已完成本地实现与自测；当前公网端口为 `18110`，真实 SMTP/CloudMail 环境验收、CDK 生产闭环、生产部署点验及调用方迁移仍待完成
- 制定日期：2026-08-10
...
| POP3 监听、CloudMail `emailList` 转换 | 已完成（本地验收） | 已接入 FastAPI lifespan，默认监听容器内 `8110`，当前公网映射端口为 `18110`；普通用户/管理员授权、邮箱归属校验、CloudMail `emailList` 和 RFC822 转换已实现；POP3 聚焦测试已通过 |
| 管理员全量邮箱访问、积分规则和审计 | 已完成（本地验收） | 管理员全局 POP 授权可通过 POP3 `18110` 读取全部可查询邮箱；用户停用/授权码清除、积分调整/规则配置和管理 API 审计已接入；当前 HTTP 管理 API 只提供邮箱记录和请求日志查询，不宣称存在邮件内容、刷新、释放或邮箱 POP 开关子接口 |
...
| 管理端页面与权限入口 | 已完成（本地验收） | 管理端已增加用户管理、积分/POP 设置，保留实例、域名、邮箱记录和请求日志页面；用户、邮箱和日志表格已改为紧凑布局，邮箱用途/来源和域名/实例单行展示，请求日志支持关键词/状态筛选并展示用户邮箱、用户名、调用密钥名称、用户 ID、业务错误码和中文错误信息；管理员全局 POP 授权码按明文保存，支持手动输入、明文显隐、留空自动生成，并可随时查看、复制和修改当前值；用户级调用密钥仅在用户中心管理 |
| 容器公网端口映射和镜像端口声明 | 已完成（本地验收） | Compose 保留 `110:8110`、启用 `18110:8110` 并暂存 `995:8110`；`/healthz` 暴露 `pop3Listening`，健康检查验证 HTTP 与容器内 `8110`；生产使用 `18110/tcp`，`995` 映射不代表支持 POP3S |
| 文档状态与验收记录 | 已完成（文档与自测） | 对外/规划文档已与当前路由、静态入口、Docker 映射、邮箱验证码注册和 POP3 `18110` 运行方式对齐；后端全量测试、注册 API 闭环和前端构建均通过；真实 SMTP 发信、CloudMail 验收及生产部署点验仍单独保留 |
| CDK 套餐、批量生成、禁用与用户兑换 | 已完成（本地验收） | 新增积分套餐和 CDK 数据表；默认预置 10000/1000 积分套餐及购买链接；管理端支持套餐维护、批量生成、筛选和禁用，用户端支持一次性兑换并原子增加积分；并发兑换同一 CDK 只允许一个成功 |

本方案建设一个独立的完整邮箱网关。图片站、Kirox、Windows EXE 及其他调用方只对接 Xiaoasi Mail Gateway，不了解 CloudMail 的管理员凭据、Token、接口路径和响应结构。

截至 2026-08-11 已完成：服务器 PostgreSQL 持久化、多实例独立 Token、域名路由、幂等建箱、邮箱会话凭证、验证码查询、管理端登录、实例和域名管理、邮箱记录、请求日志、Docker 多阶段构建及数据保留清理脚本。本次规划调整为引入独立用户体系：普通用户拥有自己的登录账号、用户级调用密钥、用户级 POP 授权码和积分余额；唯一管理员拥有独立的全局 POP 授权码，可以读取全部未物理删除且上游仍存在的邮箱；邮箱记录归属用户，创建邮箱按配置扣除积分；同时提供单一只读 POP3 代理，通过 CloudMail HTTP `emailList` 获取邮件。当前服务器公网使用 `18110`，容器内部仍监听 `8110`。当前核心代码、用户中心、旧全局 `client-keys` 清理、POP3、Docker 映射和文档核对已完成，剩余工作是使用真实 CloudMail 环境验收、图片站/Kirox Provider 迁移以及生产宝塔点验。
...
15. 唯一管理员可以管理全部用户、CloudMail 实例、域名、积分规则和积分流水；可以通过管理 API 查看全部邮箱记录和请求日志，并通过管理员全局 POP 授权码在 POP3 `18110` 读取全部可查询邮箱内容。
16. 网关不提供 CloudMail Token 外发接口，调用方只能使用统一邮箱业务接口。
17. 本期只提供单一普通明文 POP3 只读代理，不实现完整邮件服务器能力；当前公网端口为 `18110`，不支持 POP3S/TLS。
18. 外部业务继续使用单邮箱 `POST /v1/mailboxes`；用户中心额外提供 `POST /user-api/mailboxes/batch` 批量预建 1 至 50 个 POP 邮箱，所有邮箱仍共用所属用户的 POP 授权码。
19. POP 客户端通过固定的 POP 主机和服务端下发的 `18110` 端口连接网关，不提交 POP 主机和端口 JSON 参数。
...
当前服务器厂商禁用 `110` 和 `995`，因此客户端使用普通 POP3 `18110`。容器内部监听 `8110`，Compose 保留 `110:8110`、启用 `18110:8110` 并暂存 `995:8110`。本期只实现普通 `USER/PASS`，不把 `STLS` 或 POP3S 纳入验收；`995:8110` 只是端口占位，不代表支持隐式 TLS。
...
- 调用方自行指定完整邮箱地址；
- 未经邮箱授权向调用方开放完整邮件正文；
- POP3S、POP3 `STLS`、SMTP 发信、IMAP 服务和任意第三方邮箱协议代理；
- 邮件删除、附件下载和长期邮箱管理；
- 多容器副本和分布式锁；
- Redis、消息队列和多副本分布式协调；
- 多管理员角色、复杂 RBAC、组织和团队权限；
- 在线支付、支付回调、自动发卡、订单核验、优惠券和自动续费；本期 CDK 只保存管理端配置的购买链接，不接入支付系统。
...
HTTP 业务客户端只认识 Xiaoasi Mail API 和用户自己的 `X-API-Key`；POP 客户端只认识 POP3 主机、服务端下发的公网端口、邮箱和用户级 POP 授权码。CloudMail 是网关内部的第一个 Provider 实现，未来新增其他邮箱服务时，不改变客户端主流程。
...
`POP3_PORT` 控制容器内部监听端口，`POP3_PUBLIC_HOST` 和 `POP3_PUBLIC_PORT` 控制用户中心展示的公网连接参数。Compose 将 `18110` 映射到容器 `8110`；本期不提供 STLS 或 POP3S。

公网通过 Docker 或宿主机端口映射提供 `18110 -> 8110`，客户端使用服务端下发的 `18110`，应用进程监听 `8110`。邮箱创建时根据域名路由选择 CloudMail 实例，并将最终的 `instance_id` 持久写入邮箱记录；POP 登录和邮件查询只使用邮箱记录中的 `instance_id`，不根据客户端提交的主机、端口或后续变更的域名动态选择上游地址。
...
- 在“我的邮箱”中批量创建 1 至 50 个 POP 邮箱，并直接查看成功结果。

用户不能查看或修改 CloudMail 实例、域名、其他用户、系统积分规则、请求日志和管理员配置。

用户级 POP 授权码由用户中心使用浏览器安全随机数自动生成，用户不手动输入或重复确认；完整值按明文保存，用户之后登录仍可查看和复制。用户中心同时从服务端自动读取 `POP3_PUBLIC_HOST`、`POP3_PUBLIC_PORT` 和当前用户可用邮箱地址，直接组成客户端连接参数；当前生产端口为 `18110`。
...
用户中心批量创建复用同一单邮箱服务，每个批次最多 50 个、服务端最多 5 路并发；每一项使用独立且可重放的幂等键，逐个执行积分预扣、上游创建、确认或退款。批次允许部分成功，已经成功的邮箱不因其他项失败而回滚，并立即出现在当前用户邮箱列表中。
...
客户端直接连接网关的固定 POP3 地址和 `18110` 端口：
...
当前 `18110` 连接中的邮箱和授权码不经过 TLS 加密，部署时必须在防火墙、访问来源和网络边界上限制暴露范围；如果调用方环境要求加密，需等后续 `STLS` 或独立 TLS 入口完成后再启用。
...
- 当前使用普通 POP3 `18110`，不提供 POP3S；首期只实现普通 `USER`/`PASS`，`STLS` 留作后续扩展；
- 只读，不支持 `DELE`；
- 不处理附件；
...
- 普通用户可以使用自己的 `userAuthCode` 读取名下状态为 `active` 或 `expired` 且邮箱 `pop_enabled=true` 的邮箱；`expires_at` 只表示 HTTP `mailboxToken`/验证码 API 会话过期，不限制 POP3 读取；
- 状态为 `released`、POP 已关闭、归属其他用户或上游邮箱已物理删除时，普通用户不能通过 POP 登录。
...
用户中心不显示 CloudMail 实例配置、其他用户、请求日志和系统配置。用户邮箱列表展示地址、用途/来源、域名、状态、创建时间、过期时间和已识别验证码，支持按邮箱/域名/来源搜索，并按用途、邮箱状态、验证码状态筛选；列表提供批量创建入口，一次创建 1 至 50 个并在完成后刷新。用户授权码页面可以查看自己的完整授权码和自动加载的 POP3 连接参数，但不展示内部实例凭据、邮箱内部密码或完整邮件正文。
...
- 邮箱：批量创建、搜索和筛选自己的邮箱记录，查看状态、过期时间并复制已识别验证码；批量创建逐项扣费并允许部分成功，释放邮箱仍需经过邮箱 Token 或用户归属校验；
...
普通用户资源查询必须在 SQL 条件中带上 `user_id`，不能只在前端隐藏菜单；管理员接口统一要求唯一账号且 `role=admin`。管理员查询全部邮箱时允许不加用户过滤；当前 HTTP 管理 API 只提供邮箱记录列表和请求日志查询，邮件正文通过 POP3 `18110` + 管理员全局授权码读取。
...
当前支持按关键词和状态分组筛选：关键词可匹配接口、业务错误码、中文错误信息、调用密钥名称、用户名或用户邮箱；状态分组包含成功、客户端错误和服务端错误。列表采用紧凑布局，时间保持单行展示；错误列同时展示业务错误码和接口返回的脱敏中文错误说明，例如 `NO_AVAILABLE_DOMAIN / 当前没有可用邮箱域名`：
...
`POP3_PORT=8110` 是容器内监听端口，公网 `18110` 由 Compose 的 `18110:8110` 映射提供。`POP3_PUBLIC_PORT=18110` 用于用户中心展示；本期不提供 STLS 或 POP3S，`995:8110` 只是暂时保留的明文端口映射。
...
单容器首版采用单 Uvicorn worker：应用 lifespan 启动时创建 asyncio POP3 server，监听容器内 `8110`；应用停止或重载时先停止接收新连接，再等待现有连接释放。Dockerfile 声明 `EXPOSE 8110`，Compose 将当前公网 `18110` 映射到 `8110`。当前 Compose 健康检查同时验证 HTTP `/healthz` 的 `pop3Listening` 状态和容器内 `8110` TCP 连接；不得通过普通 HTTP 反向代理转发 POP3，也不得启动多个会争抢 `8110` 的 worker。
...
POP3 调用方不调用网关 HTTP 邮件查询接口，而是按标准邮件客户端方式连接固定的 POP3 主机和当前公网 `18110` 端口：

1. 使用用户自己的 `X-API-Key` 调用 `POST /v1/mailboxes` 创建单个邮箱，或在用户中心批量预建 POP 邮箱；
2. 在用户中心自动生成并安全保存用户级 `userAuthCode`；
3. 保存响应中的 `address` 和 `mailboxToken`；
4. 在邮件客户端中配置固定的 POP3 主机、18110 端口、邮箱和用户级授权码；
...
- 实现积分规则配置和用户积分手工调整；
- 实现邮箱记录和请求日志只读页面；
- 实现管理员全量邮箱记录查询；管理员邮件内容通过 POP3 `18110` + 全局授权码读取；释放、禁用、重新查询等 HTTP 管理操作留作后续扩展；
- 实现系统设置和审计日志。
...
- 自动创建邮箱闭环；
- 用户中心批量创建 1 至 50 个邮箱、部分成功、积分不足和同批次幂等重试；
- 指定域名创建；
- 跨实例候选域名失败切换；
- 相同幂等键不重复建箱；
...
- 标准 POP3 客户端可以通过固定主机和 18110 端口连接；
...
- 普通用户使用自己的授权码可以读取名下 `active` 和 `expired` 且启用 POP 的邮箱；
- 普通用户读取已过期邮箱时不受 HTTP `mailboxToken` 会话期限影响；
- 已释放、POP 已关闭和归属不匹配的邮箱不能读取；
- 普通用户授权码不能读取其他用户邮箱；
- 过量连接和登录失败会被限制；
...
- 可以看到实例和域名健康状态；
- 可以查看邮箱记录和脱敏错误；
- 可以在服务器配置 POP3 公网主机和端口；当前端口为 18110，STLS 证书配置仅作为后续扩展预留；
...
- 用户可以批量创建自己的 POP 邮箱，成功结果自动进入列表且每个邮箱正确扣费；
...
23. 外部业务使用 `POST /v1/mailboxes` 创建单个邮箱；用户中心可使用 `POST /user-api/mailboxes/batch` 预建 1 至 50 个 POP 邮箱并直接查看结果。
24. 标准 POP3 客户端可以使用固定的 POP 主机和 18110 端口连接网关。
...
33. 对外文档明确 POP3 主机、18110 端口、用户名、普通用户授权码和管理员全局授权码使用方式。
...
创建成功确认扣费，失败则写入退款流水
                ↓
用户在邮件客户端配置 POP 主机、18110 端口、邮箱和 userAuthCode
                ↓
客户端通过 POP3 TCP 连接网关 18110 端口
                ↓
...
CloudMail 将成为网关内部可替换的 Provider。普通邮件客户端只依赖 POP3 主机、服务端下发的公网端口、邮箱和用户级授权码；管理员邮件客户端使用相同 POP3 主机、端口、任意邮箱地址和管理员全局授权码。当前公网端口为 `18110`。所有客户端均不感知 CloudMail HTTP、Token、管理员凭据和实例路由；用户注册、调用密钥、积分和管理员全量访问均由网关统一管理。
